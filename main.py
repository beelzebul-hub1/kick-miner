...
from render_reporter import start_reporter
import web_server
...
async def main():
    ...
    # 5b. Render Dashboard reporter (pushes points to your external website)
    render_cfg = config.get("RenderDashboard", {})
    start_reporter(
        account_manager,
        url=render_cfg.get("url", "https://render-1-ethy.onrender.com/api/update"),
        interval=render_cfg.get("interval", 30),
        enabled=render_cfg.get("enabled", True),
    )
